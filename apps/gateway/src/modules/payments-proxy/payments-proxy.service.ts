...
const buildProxyRequestHeaders = (incomingHeaders: Headers, requestId: string) => {
  ...
  // Forwarded as-is (including Stripe-Signature) so the orders service can
  // verify the webhook signature against the unmodified raw body.
...
/**
 * Public passthrough proxy for the Stripe webhook. Unlike the cart/orders
 * proxies it performs no JWT check and injects no identity headers — the
 * request is authenticated downstream by the Stripe signature over the raw body.
 */
export const createPaymentsProxyService = (ordersServiceUrl: string) => {
  return {
    async proxyRequest(c: Context) {
      const requestId = c.get('requestId');
      const incomingUrl = new URL(c.req.url);
      const targetUrl = new URL(`${c.req.path}${incomingUrl.search}`, ordersServiceUrl);
