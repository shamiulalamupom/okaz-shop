...
export const getOrderController = async (c: Context) => {
  ...
  const order = await ordersService.getForUser(c.req.param('id')!, user.id, isAdmin);
...
export const createOrderController = async (c: Context) => {
  ...
  try {
    const order = await ordersService.create(user.id, parsed.data, requestId);
...
export const cancelOrderController = async (c: Context) => {
  ...
  const result = await ordersService.cancel(c.req.param('id')!, user.id, isAdmin, requestId);
